...
SERVER_ITEM_ADD = 'SERVER-ITEM-ADD'
...
#22. 5.11 - fotinet 관련 기능 추가
ZABBIX_HOST_CREATE = 'ZABBIX_HOST_CREATE'
ZABBIX_HOST_DELETE = 'ZABBIX_HOST_DELETE'
ZABBIX_HOST_UPDATE = 'ZABBIX_HOST_UPDATE'

ZABBIX_HOST_SNMP_CHECK = 'ZABBIX_HOST_SNMP_CHECK'
...
class MonManager(RequestHandler):
    ...
    def post(self):
        ...
        elif self.opCode == SERVER_ITEM_ADD :
            res = self.serverItemAdd()
        ...
        elif self.opCode == ZABBIX_HOST_CREATE:
            res = self.zabbix_host_create()
        elif self.opCode == ZABBIX_HOST_DELETE:
            res = self.zabbix_host_delete()
        elif self.opCode == ZABBIX_HOST_UPDATE:
            res = self.zabbix_host_update()
        elif self.opCode == ZABBIX_HOST_SNMP_CHECK:
            res = self.zabbix_host_snmp_check()
    ...
    def serverItemAdd(self):
        """
        - FUNC: 감시 아이템 Add ( Ping 기능 추가)
        - OUTPUT : 
            result: WEB 용 응답
        """
        try:
            reqdata = json.loads(self.request.body)
            tid = reqdata['tid']
            rres = mon_handler.addItemPing( tid, reqdata, self.dbm )
        except Exception, e:
            rres = rrl.rFa(None, rrl.RS_EXCP, e, None, self.request.body)
            logger.error( rres.lF(self.opCode) )
            logger.fatal(e)
    ...
    def zabbix_host_create(self):
        ...
        try:
            ...
            rres = mon_handler.zabbix_host_create( tid, reqdata, self.dbm, self.cfg )
    ...
    def zabbix_host_delete(self):
        ...
        try:
            reqdata = json.loads(self.request.body)
            tid = reqdata['tid']
            rres = mon_handler.zabbix_host_delete( tid, reqdata, self.dbm, self.cfg )
        except Exception, e:
            rres = rrl.rFa(None, rrl.RS_EXCP, e, None, self.request.body)
            logger.error( rres.lF(self.opCode) )
            logger.fatal(e)
    ...
    def zabbix_host_update(self):
        ...
        try:
            reqdata = json.loads(self.request.body)
            tid = reqdata['tid']
            rres = mon_handler.zabbix_host_update( tid, reqdata, self.dbm, self.cfg )
        except Exception, e:
            rres = rrl.rFa(None, rrl.RS_EXCP, e, None, self.request.body)
            logger.error( rres.lF(self.opCode) )
            logger.fatal(e)
    ...
    def zabbix_host_snmp_check(self):
        """
        - FUNC: snmp 장비 SNMP 체크 
        - OUTPUT : 
            result: WEB 용 응답
        """
        try:
            reqdata = json.loads(self.request.body)
            tid = reqdata['tid']
            rres = mon_handler.zabbix_host_snmp_check( tid, reqdata )
        except Exception, e:
            rres = rrl.rFa(None, rrl.RS_EXCP, e, None, self.request.body)
            logger.error( rres.lF(self.opCode) )
            logger.fatal(e)
...
def url( _cfg ):
    ...
    url = [
            ...
            ('/server/item/add', MonManager, dict(opCode=SERVER_ITEM_ADD, cfg=_cfg)),             # 통계 데이터 생성을 위한 감시항목 선택 또는 선택해제

            ('/server/item/mod/select', MonManager, dict(opCode=STATIC_ITEM_SELECT, cfg=_cfg)),             # 통계 데이터 생성을 위한 감시항목 선택 또는 선택해제

            ('/zabbix/host/create', MonManager, dict(opCode=ZABBIX_HOST_CREATE, cfg=_cfg)),             # SNMP 장비 등록
            ('/zabbix/host/delete', MonManager, dict(opCode=ZABBIX_HOST_DELETE, cfg=_cfg)),             # SNMP 장비 삭제
            ('/zabbix/host/update', MonManager, dict(opCode=ZABBIX_HOST_UPDATE, cfg=_cfg)),             # SNMP 장비 수정
            ('/zabbix/host/snmp_check', MonManager, dict(opCode=ZABBIX_HOST_SNMP_CHECK, cfg=_cfg)),             # SNMP 장비 SNMP 상태 체크
