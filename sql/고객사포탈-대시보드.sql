SELECT cnf.serverseq,
       ...
       perf.c6,
       CASE
           WHEN cnf.name = 'KT-UTM'
                OR cnf.name = 'KT-VNF' THEN 1
           ELSE 2
       END AS ord,
       nms.c8,
       perf.c9,
...
FROM
 ...
 1 as version,
 0 as vdudseq,
 'AXGATE-PNF' as description,
  web_url as weburl,
 'AXGATE-PNF' AS nf_type
   FROM tb_server 
   WHERE serverseq = 1605 ) AS cnf -- 성능정보
LEFT OUTER JOIN
  (
  SELECT 0 as vdudseq ,
          CASE
   ...
   FROM
     ...
     SELECT CASE
      ...
      WHERE s.serverseq = 1605 ) AS perf
...
LEFT OUTER JOIN
  ...
  SELECT 0 as nsseq,
   ...
   WHERE s.serverseq = 1605
            ) AS alarmcnt ON cnf.nsseq = alarmcnt.nsseq
--AND cnf.vdudseq = alarmcnt.vdudseq -- 2017.03.30 c8 컬럼 표시를 up에서 1/1으로 변경
LEFT OUTER JOIN
  (
  SELECT c8,
          0 as nsseq
